...
source("code/13_gwas_functions.R")
```

# Introduction

In this file I analyse the results of the GEMMA association analysis and BSLMM 
performed on the traits of the P. axillaris - P. exserta natural hybrid
population.
...
mh <- function() {manhattan(assoc_df, pval = p, paste0(basename, "_", p), 0.95, correct = TRUE)}
...
qqdf <- data.frame(phenotype = rep("Tube length", times = length(y)),
...
y <- rev(sort(-log10(assoc_df[ , "p_lrt"])))
# save the logP in a new df to plot all the P in the same qqplot
tmp <- data.frame(phenotype = rep("Pistil length", times = length(y)),
                  model = rep("lm", times = length(y)),
                  p_obs = y,
                  p_the = x,
                  stringsAsFactors = FALSE)
...
```{r lm4_morpho4}
basename <- "hardfiltered_biallelic_cr09_mm005_pheno4_lm4"
assoc_df <- read.table(paste0("data/raw/gwas/output/", basename, ".assoc.txt"),
                    header = T,
                    as.is = "rs")
# choose p value test
...
qq <- function() {myqqplot(assoc_df, p = p, paste0(basename, "_", p))}
mh <- function() {manhattan(assoc_df, pval = p, paste0(basename, "_", p), 0.95, correct = TRUE)}
png(paste0("figures/exploratory/gwas/plot_qq_manhattan_lm_pheno4_", p, ".png"),
    width = 1600, height = 300)
layout(matrix(c(1,2,2), 1, 3, byrow = TRUE))
qq()
mh()
...
y <- rev(sort(-log10(assoc_df[ , "p_lrt"])))
# save the logP in a new df to plot all the P in the same qqplot
tmp <- data.frame(phenotype = rep("Tube length", times = length(y)),
...
```{r lmm4_pheno4}
basename <- "hardfiltered_biallelic_cr09_mm005_pheno4_lmm4"
assoc_df <- read.table(paste0("data/raw/gwas/output/", basename, ".assoc.txt"),
                    header = T,
                    as.is = "rs")
# choose p value test
...
qq <- function() {myqqplot(assoc_df, p = p, paste0(basename, "_", p))}
mh <- function() {manhattan(assoc_df, pval = p, paste0(basename, "_", p), 0.95, correct = TRUE)}
png(paste0("figures/exploratory/gwas/plot_qq_manhattan_lmm_pheno4_", p, ".png"),
    width = 1600, height = 300)
layout(matrix(c(1,2,2), 1, 3, byrow = TRUE))
qq()
mh()
...
basenames <- c("hardfiltered_biallelic_cr09_mm005_pheno1_bslmm1",
               ...
               "hardfiltered_biallelic_cr09_mm005_pheno4_bslmm1")
```
...
From the parameter file we extract the sparse effect size beta and we multiply 
it by gamma, because beta is not just b, but it's beta when gamma == 1.

We also get variants with high Posterior Inclusion Probability (PIP = gamma).
...
for (set in basenames) {
  ...
  params <- as.data.frame(fread(paste0("data/raw/gwas/output/", set, ".param.txt"),
                                header = T, sep = "\t"))
  #  add sparse effect size (= beta * gamma) to data frame
  params["eff"] <- abs(params$beta * params$gamma)
  # Get variants with sparse effect size > 0
  params.effects <- params[params$eff > 0, ]
  print(paste0(nrow(params.effects), " variants have sparse effect size > 0."))
  # check if there are variants with effect > 0
  if ( nrow(params.effects) > 0 ) {
    # Sort by decreasing effect size
    params.effects <- params.effects[order(-params.effects$eff), ]
    # top 1% variants (above 99% quantile)
    top1 <- params.effects[params.effects$eff > quantile(params.effects$eff, 0.99), ]
    # write table
    write.table(top1, file = paste0("data/raw/gwas/output/", set, "_top1eff.dsv"), quote = F, row.names = F, sep = "\t")
    assign(paste0(set, ".params"), params.effects)
  }
  ...
  print(paste0(nrow(pip01), " variants have gamma >= 0.01."))
  if ( nrow(pip01) > 0 ) {
    write.table(pip01, file = paste0("data/raw/gwas/output/", set, "_pip01.dsv"), quote = F, row.names = F, sep = "\t")
  }
}
```

Plot variant PIPs across linkage groups/chromosomes. Only variants with effect
higher than 0 are shown.

```{r}
for (set in basenames) {
  if ( exists(paste0(set, ".params")) ) {
    params <- get(x = paste0(set, ".params"))
  png(file = paste0("figures/exploratory/gwas/", set, "_pip_plot.png"),
      width = 11.7,
      height = 7.6,
      units = "in",
      res = 200)
  pip_plot(params, title = set)
  dev.off()
  }
}
