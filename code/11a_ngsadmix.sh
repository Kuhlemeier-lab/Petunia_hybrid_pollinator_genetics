...
# save the IDs of individuals in group <= 0.10 and >=0.90 for later use in the
# divergence analysis
# admixture group 1, adm >= 0.90
admixture09 <- "admixture,09,"
admixture09members <- paste0(q2$id[q2$Q2.1 >= 0.90], collapse = ",")
admixture09str <- paste0(admixture09, admixture09members, collapse = "")
# admixture group 2, adm <= 0.10
admixture01 <- "admixture,01,"
admixture01members <- paste0(q2$id[q2$Q2.1 <= 0.10], collapse = ",")
admixture01str <- paste0(admixture01, admixture01members, collapse = "")

myfile <- "data/raw/samples_in_admixture_groups.csv"
cat(admixture09str, file = myfile, append = FALSE, sep = "\n")
cat(admixture01str, file = myfile, append = TRUE, sep = "\n")

x <- paste0(q2$id[q2$Q2.1 < 0.10], collapse = ",")
